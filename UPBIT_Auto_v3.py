...
app = Flask(__name__)
@app.route('/')
def home(): return "UPBIT Testing Bot (No Dust Sell) is Running!"
...
send_telegram("🚀 [테스트 가동] 24시간 연중 무휴")
...
while True:
    try:
        ...
        for item in portfolio:
            ...
            # [추가 조건] 보유 총 금액이 5,000원 미만이면 매도 대상에서 제외
            ...
            if profit_rate >= 2.0 and trade_state[ticker]['stage'] == 0:
                ...
                if (sell_amt * curr_p) >= MIN_ORDER_AMOUNT:
                    ...
                    # 30% 금액이 5,000원 미만이면 이번 단계 스킵 (다음 단계에서 합쳐서 매도)
                    trade_state[ticker]['stage'] = 1
