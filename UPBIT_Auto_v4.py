...
telegram_chat_id = "8403406400" # 챗 아이디는 그대로 씁니다
...
# ==========================================
# 🌐 가짜 웹 서버 방어막 (Render Timed out 방지)
# ==========================================
app = Flask(__name__)
...
def run_server():
    # Render가 요구하는 포트(보통 10000)를 열어줍니다.
...
if __name__ == "__main__":
    # 1. 가짜 웹 서버를 별도의 스레드(백그라운드)에서 실행시킵니다.
    Thread(target=run_server, daemon=True).start()
    
    send_message("== 쿠퍼춘봉 스캘핑 봇 V4.1 가동 ==\n(Render 방어막 가동 & 초단타 모드)")
    target_list = get_target_tickers()

    # 2. 봇은 메인 스레드에서 무한 루프를 돕니다.
    while True:
        try:
            now = datetime.datetime.now()
